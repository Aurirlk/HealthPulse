...
| 文档 | 路径 | 说明 |
|------|------|------|
| 需求分析 | `requirements/requirements-analysis.md` | 功能需求、用例图、流程图 |
| 数据库设计 | `database/database-design.md` | ER 图、建表 SQL、字段说明 |
| 后端开发 | `backend/backend-development.md` | 接口设计、核心代码、架构说明 |
| 前端开发 | `frontend/frontend-development.md` | 页面设计、组件说明、路由设计 |
| 测试文档 | `testing/test-report.md` | 测试用例（24条）、测试报告 |
| Linux部署 | `deployment/linux-deployment.md` | 环境搭建、部署命令、常见问题 |
| AI工具使用 | `ai-usage/ai-tool-usage-record.md` | AI工具使用记录（≥500字） |
...
| 编号 | 缺陷 | 影响 | 位置 |
|------|------|------|------|
| D-001 | 部分 SQL 脚本未执行 | 扩展功能不可用 | `Data/sql/` |
...
| 编号 | 缺陷 | 影响 | 位置 |
|------|------|------|------|
| D-003 | 旧页面 UI 未按小红书风格重构 | 视觉不一致 | `views/` |
| D-004 | WebSocket 未实际部署测试 | 消息通知可能不可用 | `websocket/` |
| D-005 | 缺少单元测试 | 测试覆盖率不足 | `src/test/` |
...
| D-009 | 部分接口缺少输入验证 | 安全性不足 | `controller/` |
...
**总计**: 30+ 张表 | 100+ 个API | 35个前端页面 | 33个core模块 | 8份文档
